...
required = [
    ...
    PROJECT_ROOT / "docs/backend/P1_ACCEPTANCE.md",
...
if "edit_version" not in post_model or '"version_id_col": edit_version' not in post_model:
    raise SystemExit("Post optimistic version mapping missing")
...
manifest = subprocess.run(
    [sys.executable, ROOT / "scripts/generate_manifest.py", "--check"],
    cwd=ROOT,
    text=True,
    capture_output=True,
)
if manifest.returncode != 0:
    raise SystemExit(manifest.stdout + manifest.stderr)
print(manifest.stdout.strip())
manifest_text = (ROOT / "MANIFEST.sha256").read_text(encoding="utf-8")
