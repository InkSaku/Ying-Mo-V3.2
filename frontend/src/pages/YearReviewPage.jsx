...
export function YearReviewPage() {
  ...
  return <main className="page-shell year-review-page" aria-busy={state.loading || undefined}>
    ...
    <section className="content-section"><h2>月份轨迹</h2><div className="year-review-months">{data.months.map((item) => <div key={item.month} className={item.total ? "active" : ""}><span>{item.month} 月</span><strong>{item.total}</strong><small>{item.article} 篇文章 · {item.note} 则随记</small></div>)}</div></section>
