...
游客可直接访问的前端路由仅包括 `/`、`/login`、`/register`、找回密码、邮箱验证和 `/about`。Article、Note、Collection、用户主页、Category、Tag、Archive、Search、互动、个人中心、创作与后台路由全部要求先确认有效登录；认证状态尚未确认时只显示加载状态，不提前请求内容。
