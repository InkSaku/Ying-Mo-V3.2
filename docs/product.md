...
> 范围：V3.2 基线及 V3.3–V3.10 增量规则；版本段落不等于上线证明
...
视觉、字体、颜色、组件及页面布局统一由 `docs/frontend/DESIGN.md` 定义；本文中的旧导航与页面结构作为功能入口清单理解，不再作为独立视觉规范。需求归本文，验证结果归验收记录，不能相互替代。
...
## 9.12 V3.3 Collection 时间轴

Collection 详情提供按真实时间重新阅读共同记录的时间轴视图：

- Article 使用 `published_at`；
- Note 使用 `occurred_at ?? published_at`；
- published 与当前仍可读的 archived Post 均可进入时间轴；
- 支持按年份、作者和 Article / Note 类型筛选；
- 年份计数、作者选项、总数和分页必须先应用当前 Collection ACL；
- 同一语义时间下按 Post ID 倒序稳定排列。

时间轴是 Collection 的阅读视图，不改变正文、作者、发布状态或 Collection 内既有手动展示顺序。

## 9.13 V3.3 共同回忆媒体墙

Collection 成员可浏览当前 Collection Post 绑定的有效图片和 Live Photo 静态画面。媒体墙要求：

- 只返回 `active` 且未删除的媒体；
- 不返回 Live Photo 视频文件作为独立缩略图；
- 每个媒体条目必须保留原 Post、真实作者和语义时间；
- 点击媒体回到原 Article / Note；
- 支持与时间轴一致的年份、作者和类型筛选；
- 媒体读取继续经过受保护代理和 Collection ACL，不生成公开对象存储 URL。

## 9.14 V3.3 关键记录

Collection creator 可以从当前可展示 Post 中选择最多 6 条关键记录并调整顺序。关键记录只影响 Collection 的回忆入口：

- creator 不能借此编辑、删除或改写其他作者正文；
- member 不能修改关键记录；
- Post 移出、移动或 Collection 删除时必须清除其关键记录状态；
- 隐藏、删除、草稿或已失效 Post 不得继续作为关键记录返回；
- 关键记录不会改变时间轴的真实时间顺序。

## 9.15 V3.3 验收标准

- creator 与 member 看到完全基于同一 Collection ACL 的时间轴、年份计数和媒体墙；
- 非成员访问 Collection、时间轴或媒体墙统一返回 404；
- 成员被移除后立即无法读取上述入口；
- 时间轴筛选可组合、可分页并可通过 URL 恢复；
- 关键记录最多 6 条且仅 creator 可保存顺序；
- 图片、Live Photo、地点、心情和真实作者信息在窄屏与桌面均可安全展示。

## 9.16 V3.4 未来成员自动加入

Collection creator 可选择是否让未来成功注册的 Ying-Mo 成员自动加入该 Collection。该设置遵循现有统一 ACL：自动加入后同时获得阅读权与投稿权。

- 默认关闭，仅 creator 可在创建或管理 Collection 时修改；
- 只影响开关开启后成功注册的新成员，不补加此前已有成员；
- 关闭后仅停止后续自动加入，不移除已经加入的成员；
- “选择当前所有成员”仍是当前时刻快照，与该开关互不替代；
- 注册与自动成员关系必须在同一数据库事务中提交，失败不得遗留孤立关系或通知；
- 只处理未删除、正常可用且 creator 账号正常的 Collection；
- 自动成员与手动成员权限一致，creator 可继续手动移除；被移除后不得因登录而重新加入；
- 自动加入必须记录成员来源并发送明确说明“可阅读并投稿”的站内通知；
...
- 登录态桌面导航与移动菜单始终提供通知入口；有未读通知时显示 `1–99 / 99+` 数量角标；
- 使用独立轻量接口返回当前用户未读总数，不为角标请求完整通知列表或个人概览；
- 登录后的第一次读取只建立未读基线，不弹出历史通知；
- 页面重新获得焦点、从后台恢复、通知已读状态变化及约 60 秒轮询时刷新未读数；
- 当前会话内未读数增加时显示可关闭、可跳转的轻量 Toast，不使用声音、系统通知或闪烁；
- 用户已停留在通知中心时，新通知事件应就地刷新列表，不能只更新全局角标；
- 通知中心必须通过背景、文字标签和字重共同区分未读状态，不能只依赖颜色；
- “全部已读”和单条已读后，全局角标应及时回落；计数接口临时失败不得阻断正常页面与认证流程。
...
- Article 与 Note 均支持阅读统计；
- 只有进入真实详情页且页面连续处于可见状态至少 5 秒，前端才上报一次有效阅读；
- 首页、列表、Search、Archive、Collection 卡片等预览不产生阅读记录；
- 后端必须在写入阅读事件前重新执行当前 Post / Collection ACL，不能因为统计接口扩大可读范围；
- 作者阅读自己的 Post 不计入统计；
- 同一读者对同一 Post 在 30 分钟内的重复上报只计一次，并使用数据库唯一约束兜底并发重复请求；
- 阅读事件保存 `post_id`、`reader_id`、去重时间桶与 UTC 创建时间，用于聚合，不向作者返回读者身份列表；
- 作者管理面提供总阅读次数、去重读者数、近 7 天阅读次数和近 30 天阅读次数；
- 阅读统计只对 Post 作者本人通过作者管理接口可见，不在普通详情页、排行榜或成员公开页面展示；
- 成员后续失去 Collection 权限时，历史聚合记录可保留，但不得再产生新的阅读记录；
- 第一版不采集阅读时长、滚动深度、完成率或前后台心跳，不建设成员阅读监视能力。
...
V3.8 已实现范围：可安装 PWA 外壳、桌面/主屏幕应用图标、写随记/写文章/搜索快捷入口、设置页安装引导和可控版本刷新。Service Worker 仅预缓存静态前端外壳，不缓存 API、成员内容或受保护媒体；PWA 不改变登录、ACL、私有缓存和数据持久化边界，也不等同于原生 App。

V3.9 Note 阶段一已实现范围：首页内嵌 Quick Note Composer，默认仅自己可见，并可显式切换登录成员或有权 Collection；文字输入优先保存在当前成员本地，首次图片操作才创建服务器草稿；图片、地点、心情和发生时间按需展开；发布后在首页原地回显，也可携带同一服务器草稿进入完整编辑器。该阶段复用既有 Post、Upload 与 ACL 契约，不新增数据库结构，不改变 Article 正式写作链、Note 语义时间或 Collection 权限边界。

V3.9 Note 阶段二已实现范围：首页从模块集合调整为 Article / Note 混合单列时间流，统一使用 `Article.published_at` 与 `Note.occurred_at ?? published_at` 排序；`/api/v1/home/feed` 在统一 ACL 过滤后提供签名游标分页和 `all / note / article` 筛选，归档、隐藏、删除和无权内容不得进入 Feed。客户端只在内存中短时保留已加载内容，`sessionStorage` 仅保存按成员和筛选隔离的数值滚动坐标；从详情返回时恢复已加载页面和阅读位置。新发布 Note 在当前 Feed 顶部即时出现，后续刷新仍回到服务端语义时间顺序。PWA“写随记”快捷入口改为聚焦首页 Composer。

V3.9 Note 阶段三已实现范围：首页 Feed 与全站浏览卡片分别拆为 Article / Note 展示组件。Article 以标题、摘要、封面、阅读时间和正式阅读入口为主；Note 以正文、发生时间、地点、心情和现场影像为主，不再使用“未命名随记”充当标题。共享骨架只维护作者、媒体、Canonical 跳转、Collection / 地点上下文和 Feed 返回状态。列表正文来自统一 ACL 过滤后生成的最多 500 字 `content_excerpt`，完整 body 仍只在有权详情中返回；该字段不改变 Post、Collection 或媒体权限。

V3.9 Note 阶段四已实现范围：首页 Feed 以“第一条可见 Post + 相对吸顶工具栏偏移”保存阅读锚点，详情返回、`all / note / article` 筛选往返和深处插入新 Note 都优先恢复内容位置，而不依赖易受卡片高度与响应式布局影响的绝对像素。每个成员、每种筛选拥有独立的 30 分钟内存页面与锚点；首次进入未缓存筛选从 Feed 起点开始。`sessionStorage` 继续只保存隔离后的数值滚动坐标作为兜底，不保存内容、Post ID 或列表快照。本阶段不改变签名游标、ACL、Post 语义时间、API 或数据库结构。

V3.9 Note 阶段五已实现范围：Note 详情以作者与生活时间开场，有真实标题才展示标题，正文先于影像、地点、心情、发布时间与互动，不再以“随记”制造视觉标题。属于 Collection 的 Note 会展示同一 Collection 中语义时间最接近的最多 4 条可读记录，允许朋友 Note、影像和 Article 混排，并明确标注发生在当前 Note 此前或后来；独立 Note 不生成关系内容。共同经历复用统一 ACL，只包含已发布或已归档且未隐藏、未删除的内容，卡片响应不返回完整 body；该关系是 Collection 权属与时间邻近的静态解释，不是推荐算法、热度排序或用户画像。本阶段不新增数据库结构或独立 API 端点。

V3.9 Note 阶段六已实现范围：“往年今日”以低频编辑插页进入首页 `all` 时间流，而不是恢复成独立首页模块。插页最多展示 3 条 ACL 安全的历史同日记录，固定出现在第三条当前内容之后，使用日期、年份、作者、类型、短内容和可选首图；`note / article` 筛选不混入回忆。已进入插页的 Post ID 随签名游标传递并从后续普通分页排除，因此同一内容在一次连续阅读中只出现一次。历史归档仍按往年今日规则可见，草稿、隐藏、删除和失权内容继续排除。该能力是由日历同日规则触发的静态编辑关系，不是推荐、热度、随机内容、用户画像或新的通知机制；不新增数据库结构、独立 API 端点或第三方依赖。

V3.10 共同回忆补全已实现范围：有权 Collection 成员可在其中一条 Article 或 Note 下，以自己的 Note 补充文字、图片、发生时间和地点；补充始终保留真实作者，并通过单独关系指向稳定根记录。共同回忆只允许一层，不形成回复树；从任一补充进入都回到同一条时间线。草稿不公开，发布时重新校验成员资格、根记录状态与编辑版本；失权或原记录不可用时保留草稿，作者可解除关联后转为独立私密内容。根记录移出 Collection、删除或 Collection 删除时关系失效，补充 Post 本身不随之删除。详情页“共同回忆”与基于时间邻近的“合集里的前后记录”分工展示，后者排除同一共同回忆中的记录，避免重复。该能力不改变 Collection ACL，不开放匿名读取，也不提供多人共同编辑同一正文。
